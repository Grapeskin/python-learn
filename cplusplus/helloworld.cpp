...
int main() {
  ...
  // 声明一个指针数组，存储三个函数的地址 cout << "Pointer array : " <<
  // parray[0](a, 3) << " : " << *(parray[0](a, 3)) << endl; cout << "Pointer
  // array : " << parray[1](a, 3) << " : " << *(parray[1](a, 3)) << endl; cout
  // << "Pointer array : " << (*parray[2])(a, 3) << " : " << *((*parray[2])(a,
  // 3)) << endl;
  ...
  // cout << "stu_name: " << stu.getName() << ", stu_age: " << stu.getAge() <<
  // endl; displayPoint(Point(11)); Point p(44, 99); cout << p.x << "," << p.y
  // << endl;
  ...
  // auto r3 = word_count.insert(map<string, int>::value_type("c", 4));
  // //重复key不会插入 cout << "r1 insert result = " << r1.second << endl; cout
  // << "r2 insert result = " << r2.second << endl; cout << "r3 insert result =
  // " << r3.second << endl; for (auto item : word_count)
  // {
  ...
  // 16. 模板
  // cout << compare(1, 3) << "," << compare('c', 'b') << "," <<
  // compare(3.14, 3.14) << endl;
...
bool s_is_empty(string &s) { return s.empty(); }
...
void fun2(int n, double (*fp)(int num)) { cout << "result: " << fp(n) << endl; }

double calc(int num) { return num * 0.5; }
